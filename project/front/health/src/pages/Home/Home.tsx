...
function ExerciseRecordChart() {
  ...
  const minTime = Math.min(...chartData.map((data) => data.time));
  ...
  const options = {
            ...
            ? [
                {
                  ...
                  xValue: chartData[clickedIndex].day,
                  yValue: chartData[clickedIndex].time,
                  content: [`${formatTime(chartData[clickedIndex].time)}`, `${selectedCalories || 0} kcal`],
                  enabled: true,
                  font: {
                    size: 10,
                    weight: 'bold' as const,
    ...
    scales: {
      ...
      y: {
        type: 'linear' as const,
        beginAtZero: false,
        min: minTime === maxTime ? minTime - 10 : minTime,
        max: maxTime === minTime ? maxTime + 10 : maxTime,
        ticks: {
          stepSize: 10,
